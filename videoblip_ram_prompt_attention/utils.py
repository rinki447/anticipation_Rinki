...
def pad_feats(feats, tokenizer):
    """Pad the features to the maximum length by 0s.
    ...
    max_feat_len = 0
    for feat in feats:
        max_feat_len = max(max_feat_len, len(feat))

    padded_feats = []
    for feat in feats:
        remainder = torch.tensor(
            [0] * (max_feat_len - len(feat))
        )
        if tokenizer.padding_side == "right":
            padded_feats.append(
                torch.cat([feat, remainder])
            )
        else:
            padded_feats.append(
                torch.cat([remainder, feat])
            )
    return torch.stack(padded_feats)

def comp_attention_mask(feats):
    """Pad the features to the maximum length by 0s.
    ...
    for ind, feat in enumerate(feats):
        attention_mask.append((feat.bool()).int())

    return attention_mask

class DataCollatorForCOTVideoSeq2Seq(DataCollatorForSeq2Seq):
    def __call__(self, features, return_tensors=None):
        if all("pixel_values" in feature for feature in features):
            pixel_values = torch.stack(
                [feature.pop("pixel_values") for feature in features]
        ...
        vid_obj_labels = (
            [feature.pop("vid_obj_labels") for feature in features]
            if "vid_obj_labels" in features[0].keys()
            else None
        )

        vid_obj_input_ids = (
            [feature.pop("vid_obj_input_ids") for feature in features]
            if "vid_obj_input_ids" in features[0].keys()
            else None
        )

        collated = super().__call__(
            features, 
            return_tensors=return_tensors
        )

        collated['vid_obj_attention_mask'] = comp_attention_mask(vid_obj_input_ids)
        
        # Pad the vid_obj_attention_mask
        collated['vid_obj_attention_mask'] = pad_feats(
            collated['vid_obj_attention_mask'], 
            self.tokenizer
        )

        # Pad the vid_obj_input_ids 
        collated['vid_obj_input_ids'] = pad_feats(vid_obj_input_ids, self.tokenizer)

        if torch.is_floating_point(collated['vid_obj_input_ids']):
            collated['vid_obj_input_ids'] = collated['vid_obj_input_ids'].to(torch.int32)

        # Pad the vid_obj_labels
        if vid_obj_labels is not None:
            collated['vid_obj_labels'] = pad_feats(vid_obj_labels, self.tokenizer)

        # torch.save(collated, "collated.pt")
        if pixel_values is not None:
            collated["pixel_values"] = pixel_values
...
def generate_input_ids_and_labels(
    ...
    prompt: str, # What is the camera wearer doing?
    text: str, # The camera wearer is performing an action "wash fork"
...
#     tokenizer: PreTrainedTokenizer, 
#     prompt: str, # "What is the camera wearer doing?"
#     text: str, # "The camera wearer performs the action: "put stone", where the verb is "put" and the noun is "stone".
#     ram_prompt:str, # The top-detected objects in this video are: {stone, rock, ...}.
...
#     :param prompt: prompt for the LLM
#     :param text: text for the LLM to generate based on the prompt
#     :param ram_prompt: prompt consisting of the top-k detected objects across the video.
#     :param decoder_only_lm: whether the LLM is decoder only or not
#     :returns: preprocessed results
#     """
#     if decoder_only_lm:
...
#         # tokenize ram_tags_prompt second
#         ram_prompt_tokens = tokenizer(ram_prompt, return_attention_mask=False).input_ids
...
#         # join tokenized prompt and narration text
#         preprocessed["input_ids"] = prompt_tokens + ram_prompt_tokens + preprocessed["input_ids"]
#         preprocessed["input_ids"] = torch.tensor(preprocessed.input_ids)

#         # # add ram_tokenized prompt to preprocessed
#         # preprocessed['ram_input_ids'] = torch.tensor(ram_prompt_tokens)

#         # for decoder only LMs, labels are same as input_ids, but we mask
#         # tokens for the prompt
#         preprocessed["labels"] = preprocessed["input_ids"].clone()
#         preprocessed["labels"][: len(prompt_tokens) + len(ram_prompt_tokens)] = -100
#     else:
#         # eos is automatically appended by the tokenizer
#         # we don't use return_tensors='pt' here b/c it automatically batchifies things
#         # which we don't want
...
def save_features(dataset, save_dir):
    num_samples = len(dataset)

    for ind in tqdm.tqdm(range(num_samples)):
        
        with torch.inference_mode():
            sample_dict = dataset.__getitem__(ind)
            seg_file = sample_dict['seg_file'].split(".pkl")[0]
            save_path = save_dir + seg_file + ".pkl"
        
            if not os.path.exists(save_path):
                with open(save_path, "wb") as f:
                    pickle.dump(sample_dict, f, pickle.HIGHEST_PROTOCOL)        

def extract_vision_features(dataset, model, save_dir):
    num_samples = len(dataset)
    ...
    for ind in tqdm.tqdm(range(num_samples)):
        ...
        with torch.inference_mode():
            sample_dict = dataset.__getitem__(ind)
            seg_file = sample_dict['seg_file'].split(".pkl")[0]
            save_path = save_dir + seg_file + ".pth"
        
            if not os.path.exists(save_path):
                pixel_values = sample_dict['pixel_values'].unsqueeze(0) # (1, 3, 8, 224, 224)
                ...
                # with open(save_path, "wb") as f:
                #     pickle.dump(vision_outputs, f, pickle.HIGHEST_PROTOCOL)
